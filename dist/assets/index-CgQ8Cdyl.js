      ...
      `).join(``),c=e.replace(/[^a-zA-Z0-9]/g,`_`);s+=`
        <div class="sp-col" id="col_${c}">
          ...
          <div class="sp-col-footer">
            ...
            <div class="col-total-val" id="total_${c}">${T(a)}</div>
          </div>
        </div>
      `}),e.innerHTML=s}}})),R=e((()=>{})),z,B=e((()=>{R(),_(),D(),c(),z={render(){let e=document.createElement(`div`);return e.className=`receivables-container`,e.innerHTML=`
    ...
    `,e},init(){this.renderGrid(),window.markReceived=e=>{g.removeReceivable(e),this.renderGrid()},window.addEventListener(`bloom:presentationToggled`,()=>this.renderGrid())},renderGrid(){let e=document.getElementById(`receivablesGrid`);if(!e)return;let t=g.presentationMode?s.receivables:g.data.receivables;if(!t.length){e.innerHTML=`<div style="grid-column:1/-1;text-align:center;padding:60px;color:var(--text-3);">🎉 Nenhum valor pendente para receber!</div>`;return}let n={};t.forEach(e=>{n[e.person]||(n[e.person]=[]),n[e.person].push(e)}),e.innerHTML=Object.keys(n).map(e=>{let t=n[e];return`
        <div class="rec-card">
          <div class="rec-card-header">
            ...
            <span class="rec-total">${T(t.reduce((e,t)=>e+t.amount,0))}</span>
          </div>
          <div class="rec-list">
            ${t.map(e=>`
              <div class="rec-item">
                <div class="rec-item-info">
                  ...
                  <span class="rec-item-val">${T(e.amount)}</span>
                </div>
      ...
      `}).join(``)}}})),V=e((()=>{})),H,U=e((()=>{_(),p(),D(),H={exportCSV(){let e=[[`Categoria`,`Valor (R$)`,`Data`],...g.getTransactions().map(e=>[e.category,e.amount.toFixed(2).replace(`.`,`,`),e.date])].map(e=>e.join(`;`)).join(`
`);f(`bloom-${u()}.csv`,`﻿`+e,`text/csv;charset=utf-8;`)},exportJSON(){f(`bloom-backup-${u()}.json`,JSON.stringify(g.data,null,2),`application/json`)},exportPDF(e){let t=g.getTransactions(),n={};t.forEach(e=>n[e.category]=(n[e.category]||0)+e.amount);let r=t.reduce((e,t)=>e+t.amount,0),i=document.getElementById(e);if(!i)return;let a=i.querySelector(`.pdf-content`);a&&(a.innerHTML=`
        ...
        <tbody>
          ${Object.entries(n).sort((e,t)=>t[1]-e[1]).map(([e,t])=>`
            <tr>
              ...
              <td style="padding:9px 10px;border-bottom:1px solid #eee;text-align:right;font-weight:600;">${T(t)}</td>
          ...
          <tr style="font-weight:800;background:#F4F4F6;">
            ...
            <td style="padding:12px 10px;text-align:right;">${T(r)}</td>
            <td style="padding:12px 10px;text-align:right;">100%</td>
          </tr>
        </tbody>
      </table>
      <p style="font-size:11px;color:#999;margin-top:20px;">Gerado por Bloom Finance em ${new Date().toLocaleDateString(`pt-BR`)}.</p>`,i.style.display=`block`,window.html2pdf?.().set({margin:12,filename:`bloom-relatorio-${u()}.pdf`,html2canvas:{scale:2},jsPDF:{unit:`mm`,format:`a4`}}).from(i).save().then(()=>{i.style.display=`none`}))}}})),W,G=e((()=>{V(),U(),W={render(){let e=document.createElement(`div`);return e.className=`reports-container`,e.innerHTML=`
    ...
    `,e},init(){document.getElementById(`btnExportCSV`)?.addEventListener(`click`,()=>H.exportCSV()),document.getElementById(`btnExportJSON`)?.addEventListener(`click`,()=>H.exportJSON()),document.getElementById(`btnExportPDF`)?.addEventListener(`click`,()=>H.exportPDF(`pdfReport`))}}})),K=e((()=>{})),q,J=e((()=>{K(),_(),h(),q={render(){let e=document.createElement(`div`);return e.className=`settings-container`,e.innerHTML=`
    ...
    `,e},init(){document.getElementById(`themeToggle`)?.addEventListener(`change`,e=>{g.settings.themeDark=e.target.checked,g.save(),document.body.classList.toggle(`theme-dark`,g.settings.themeDark)}),document.getElementById(`btnWipeData`)?.addEventListener(`click`,()=>{confirm(`Isso irá apagar todos os dados e restaurar os originais. Continuar?`)&&m.wipeData()})}}})),Y,X=e((()=>{C(),j(),P(),L(),B(),G(),J(),Y={render(){let e=document.createElement(`div`);return e.id=`appPage`,e.className=`app-page`,e.setAttribute(`aria-label`,`Workspace financeiro`),e.innerHTML=`
    ...
    `,e.querySelector(`#topbar-slot`).replaceWith(N.render()),e.querySelector(`#sidebar-slot`).replaceWith(A.render()),e.querySelector(`#page-spreadsheet`).appendChild(I.render()),e.querySelector(`#page-receivables`).appendChild(z.render()),e.querySelector(`#page-reports`).appendChild(W.render()),e.querySelector(`#page-settings`).appendChild(q.render()),e},init(){N.init(),A.init(),I.init(),z.init(),W.init(),q.init()}}}));t((()=>{n(),r(),i(),_(),S(),X(),document.addEventListener(`DOMContentLoaded`,()=>{g.init(),g.settings.themeDark&&document.body.classList.add(`theme-dark`);let e=document.getElementById(`app`);window.navigateToApp=()=>{e.innerHTML=``,e.appendChild(Y.render()),Y.init()},window.navigateToHome=()=>{e.innerHTML=``,e.appendChild(x.render()),x.init()},window.navigateToHome()})}))();